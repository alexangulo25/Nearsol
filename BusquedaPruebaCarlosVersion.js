...
define(['N/ui/serverWidget', 'N/search', 'N/record', 'N/render', 'N/config'], function (serverWidget, search, record, render, config) {
    function onRequest(context) {
        ...
        if (context.request.method === 'POST') {
            ...
            sublist.addField({ id: 'custpage_taxitem', type: serverWidget.FieldType.TEXT, label: 'Tax Item' }); // Nuevo campo para taxitem
            ...
            for (var i = 0; i < searchResults.length; i++) {
                ...
                if ((account.indexOf('witholding') !== -1 || /^15/.test(account) || /^5/.test(account) || /^6/.test(account)) && amount !== 0 && entityName) {
                    ...
                    vendorData[vendorId].lines.push({
                        ...
                        account: account,
                        amount: amount,
                        taxItem: taxItem // Agregar taxitem a los datos del vendor
                    });
                    ...
                    if (taxItem) {
                        sublist.setSublistValue({ id: 'custpage_taxitem', line: lineIndex, value: taxItem }); // Mostrar taxitem en la sublista
                    }
                    lineIndex++;
                }
            }
